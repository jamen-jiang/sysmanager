<template>
    <el-main id="main">
        <el-tabs id='tabs' type="border-card" v-model='currentTab' @tab-click="tab_click" v-if="tabs.length" @tab-remove="tab_remove">
            <el-tab-pane v-for="(item) in tabs" :key="item.name" :label="item.name" :name="item.route" :closable="item.route == '/' ? false : true " style='height:100%'>
...
export default {
    ...
    methods: {
        ...
        tab_remove(targetName) {
            ...
            if (this.currentTab === targetName) {
                // 设置当前激活的路由
                if (this.tabs && this.tabs.length >= 1) {
                    this.$store.commit('tab_active', this.tabs[this.tabs.length - 1].route);
    ...
    computed: {
        ...
        currentTab: {
            ...
            set(val) {
                this.$store.commit('tab_active', val);
            }
        }
    },
